...
local function update_stat(key, x)
    local mean_key = key .. ":mean"
    local m2_key = key .. ":m2"

    local cnt = dict_confidence:incr(key .. ":cnt", 1, 0)
    local mean = dict_confidence:get(mean_key) or 0
    local m2   = dict_confidence:get(m2_key)   or 0
    ...
    dict_confidence:set(mean_key, mean)
    dict_confidence:set(m2_key, m2)
end
